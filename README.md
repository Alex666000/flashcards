...
# memo() - оборачиваем только те компоненты которые пропсы получают от родителей
- Прогаваривать код вслух и в слух размышлять очень подробно!!!!!!!!!!!!
# Ошибки:
- сам путь запроса к серверу копируй и вставляй в браузер смотри какои ответ в браузере придет ли?
- localhost приложения вставь в инкогнито - смотри там работает? Если кеш не очищен помогает, чисть
- кеш и ЛС бзаузера и куки чисть
- смотри на UX как должно было сработать, анализируем в слух логику, смотрим нетворк
- успешно ли завершен запрос? Потом смотрим место где должны эти данные по ФЛОУ отрисоваться,
- дебаг и консоль. Анализировать что написано в строке в Headers что отправляем на сервер:
- смотреть значенеи каждого квери параметра и чтобы число было числом а строка строкой -
пример скопирали с хедеров - при этом анализировать все запросы в нетворк а не тот что я делал
если делал на добавить создать колоду - то смотреть + запрос второй что есть в нетворке
get за всеми колодами и анализировать адресную строку и значения квери параметров
- https://api.flashcards.andrii.es/v2/decks?authorId=10def95a-1df9-4259-98fb-39e587bbc854&currentPage=1&itemsPerPage=10&maxCardsCount=100&minCardsCount=0&name=&orderBy=updated-desc
#
