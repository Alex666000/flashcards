...
// дальше когда будем создавать новую апишку для карточек Cars мы также сделаем как тут const cardsApi = baseApi.injectEndpoints({}
// - и сюда писать все эндпоинты для cardsApi по примеру с decksApi
    ...
    endpoints: (builder) => {
        return {
            ...
            getDecks: builder.query<GetDecksResponse, Partial<GetDecksArgs> | void>({
                // 2 --- когда делаем get запрос мы пишем providesTags и указываем этот тэг
                // это говорит о том - что если данные сущности "МУТИРУЮТЯ", меняются, обновляются,- перевызови этот get запрос:
                // эндротинт getDecks - в котором мы прописали свойство providesTags: ['Deck'] - следит, связан с invalidatesTags: ['Deck'],
                // в эндпоинте в мутации -- связали 2 свойства разных эндпоинтов!
                // работает так — когда я делаю мутацию - в эндпоинте мутации, например в эндпоинте createDeck
                // запускается invalidatesTags: ['Deck'] и get следит и смотрит если этот запрос мутация инвалидировалась —
                // данные обновились - он сам запросит новые данные и вручную не надо обновлять их -- то есть после нажатия на кнопку
                // создания автоматом пошел второй еще плюсом get запрос к post запросу -- итог 2 запроса + новые свежие данные!!
                providesTags: ['Deck'],
                query: (arg) => ({
                    // возвращаем объект
...
// экспортируем все хуки из decksApi
