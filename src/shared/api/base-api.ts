...
// baseApi - для одной апишке для одного baseUrl -- если бы делали несколько апишек несколько микросервисов
// (несколько серверов, несколько эндпоинтов) то
// несколько апишек бы делали
// на 1 апишку 1 createApi делаем!!!
// в идеале надо было назвать ни baseApi (так назвали в учебных целях),а baseFlashCardsApi (используем как базу) -
// тут эндпоинты не используем: сделали code spliting
// - packsApi, decksApi - это будут не отдельные апи а будут отдельные части этого baseApi
// апи создается на 1 сервис - сервис это бэкенд - удаленный сервер
...
export const baseApi = createApi({
    ...
    // описание эндпоинтов запросов к серверу (перенесли в desk-api) - сделали code spliting
    // - приписка к базовому урлу - внутри пишем или квреи (для GET) или мутации для остальных
    // CRUD
    endpoints: () => ({}),
    ...
    tagTypes: ['Deck'], // чтобы подгружать новые обновленные данные, а не в ручную обновлять страницу
    // когда есть новые данные и чтобы они новые данные отрисовались на странице - надо сделать: в нашей апишке
    // обьявить тэги на которые мы будем реагировать — зарезервируем тэги на которые потом наш РТК
    // может ссылаться - назовем Deck - потом идем в конкретную апишку например в decks-api (см. п.2 файл decks-api)
})
